...
// Run: npm i express cors ollama && node tools/local-writing-service.mjs
...
const buildFeedback = ({ taskScore, grammarScore, vocabularyScore, organizationScore }) => {
  ...
  const strongPart =
    strong.length > 0
      ...
      : "Điểm mạnh: bạn giữ được giọng văn tự nhiên và ý chính đã xuất hiện.";
  const weakPart =
    weak.length > 0
      ? `Điểm cần cải thiện: ${weak.join("; ")}.`
      : "Điểm cần cải thiện: thêm ví dụ/chi tiết cụ thể để tăng độ thuyết phục.";
  const rewriteTip =
    'Thử viết lại câu khó bằng khung ngắn: ý chính + lý do + ví dụ (vd: "Working from home boosts focus because I avoid commute; for example, I start earlier and finish reports faster").';
  const flowTip =
    "Giữ mạch tự nhiên bằng từ nối (however, moreover, for example) và mở-thân-kết rõ. Khi thêm ví dụ, chọn chi tiết quen thuộc để không bị bí từ.";
  const planTip =
...
const buildStudyPlan = ({ taskScore, grammarScore, vocabularyScore, organizationScore }) => {
  const tips = [];
  if (taskScore < 4) tips.push("Đọc kỹ yêu cầu, liệt kê 2-3 ý chính trước khi viết.");
  if (grammarScore < 4) tips.push("Ôn thì cơ bản (hiện tại, quá khứ, hoàn thành), soát lỗi chủ-vị/mạo từ, ưu tiên câu ngắn.");
  if (vocabularyScore < 4) tips.push("Thêm paraphrase, collocation quen; tránh lặp từ khóa.");
  if (organizationScore < 4) tips.push("Chia đoạn rõ, mỗi đoạn 1 ý chính; dùng từ nối hợp lý.");
  tips.push("Đọc lại và sửa lỗi chính tả/ngữ pháp trước khi nộp.");
...
const snapToeicScore = (rawScore, overallScore = 0) => {
  ...
  const snapped = Math.round(clamped / 10) * 10; // TOEIC Writing đi theo bậc 10
  const band =
...
const isTruncatedFeedback = (text = "") => {
  ...
  if (/[.:;-]\s*$/.test(trimmed)) return true; // kết thúc lửng
  return false;
};
...
const systemPrompt = `
Bạn là giám khảo TOEIC Writing, chấm từng câu (0-5). Tính predictedToeicScore trong khoảng 0-200, quy đổi từ overall*40 và bấm theo các band:
...
- Feedback: giữ trong khoảng hướng dẫn (Q1-5: 100-120 từ; Q6-7: 120-150; Q8: 120-170), tiếng Việt, không song ngữ; nêu 1-2 điểm mạnh, 2-3 lỗi cụ thể (ngữ pháp/từ vựng/tổ chức), gợi ý viết lại/ví dụ ngắn, kết thúc gọn.
- StudyPlan: 3-6 gợi ý hành động ngắn gọn, dễ làm.
- Mẫu JSON:
{
  ...
  "feedback": "Nhận xét 100-170 từ, tiếng Việt",
  "studyPlan": ["Gợi ý 1", "Gợi ý 2", "Gợi ý 3"]
}
- Bắt buộc giữ đúng key, điểm nằm trong biên 0-5 và 0-200.
`.trim();

app.post("/local-writing-score", async (req, res) => {
  const { answerText = "", questionText = "", questionNumber = null, partKey = "" } = req.body || {};
  if (!answerText.trim()) {
    return res.status(400).json({ ok: false, msg: "Thiếu bài làm" });
  }
  try {
    const lengthHint =
      partKey === "w1_5" || (questionNumber && Number(questionNumber) <= 5)
        ? "Feedback 100-120 từ."
        : partKey === "w6_7" || (questionNumber && Number(questionNumber) <= 7)
          ? "Feedback 120-150 từ."
          : "Feedback 120-170 từ.";

    const userPrompt = `Question: ${questionNumber ?? "-"} | Part: ${partKey || "-"}\n${lengthHint}\nĐề bài:\n${questionText}\n\nBài làm:\n${answerText}\n\nHãy chấm và trả về JSON đúng mẫu.`;
    ...
    try {
      ...
      console.error("parse JSON error, raw response:", text?.slice(0, 400) || text, parseErr);
      parsed = {};
    }
